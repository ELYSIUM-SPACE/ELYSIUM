...
/proc/get_hat_icon(var/obj/item/hat, var/offset_x = 0, var/offset_y = 0)
	...
	if(!mob_hat_cache[key])            // Not ideal as there's no guarantee all hat icon_states
		var/t_icon
		if(hat.item_state in icon_states(GLOB.liberty_onmob_icons[slot_head_str]))			//checking for separate liberty sprites
			t_icon = GLOB.liberty_onmob_icons[slot_head_str]
		else
			t_icon = GLOB.default_onmob_icons[slot_head_str] // are unique across multiple dmis, but whatever.
