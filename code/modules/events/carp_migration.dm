	...
	while(I < n)
		...
		if(istype(T,/turf/space))
			...
			if(prob(96))
				...
				I += 3
			GLOB.death_event.register(M, src, TYPE_PROC_REF(/datum/event/carp_migration, reduce_carp_count))
			GLOB.destroyed_event.register(M, src, TYPE_PROC_REF(/datum/event/carp_migration, reduce_carp_count))
			LAZYADD(GLOB.carp_count["[Z]"], M)
			spawned_carp ++
			M.throw_at(get_random_edge_turf(GLOB.reverse_dir[dir],TRANSITIONEDGE + 2, Z), 5, speed, callback = CALLBACK(src, TYPE_PROC_REF(/datum/event/carp_migration, check_gib), M))
		I++
		if(no_show)
			break
...
/datum/event/carp_migration/proc/reduce_carp_count(var/mob/M)
	for(var/Z in affecting_z)
		...
		if(M in L)
			LAZYREMOVE(L,M)
			break
	GLOB.death_event.unregister(M, src, TYPE_PROC_REF(/datum/event/carp_migration, reduce_carp_count))
	GLOB.destroyed_event.unregister(M, src, TYPE_PROC_REF(/datum/event/carp_migration, reduce_carp_count))
