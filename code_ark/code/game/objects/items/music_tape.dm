...
/obj/item/music_tape/examine(mob/user)
	. = ..(user)
//	if(.) if(track) to_chat(user, SPAN_NOTICE("It's labeled as \"[track.title]\"."))	закомментил во время ребейза
...
/obj/item/music_tape/custom/attack_self(mob/user)
	/*if(!ruined && !track)	закомментил во время ребейза
		if(setup_tape(user))
...
//		//track = new /datum/track(new_name, sound_file)	закомментил во время ребейза
		return 1
	return 0

/obj/item/music_tape/proc/CanPlay()
	//if(!track)	закомментил во время ребейза
	//	return FALSE
