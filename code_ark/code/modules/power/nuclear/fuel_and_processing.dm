...
/obj/machinery/rod_fabricator   //Finally fixed!
	...
	var/transfering_sub
	use_power = 1
	var/obj/item/nuclearfuel/F = null
	var/load = 50
...
/obj/machinery/rod_fabricator/attackby(obj/item/Pel, mob/user)   //UI is quite outdated, but at least it works
	if(istype(Pel, /obj/item/nuclearfuel))
		src.F = Pel
...
/obj/machinery/rod_fabricator/OnTopic(var/mob/user, var/href_list, var/datum/topic_state/state)  //Buttons, displayed in UI
	if(href_list["eject"])
		if(buffer.len)
			var/obj/item/nuclearfuel/pellet/P = new(get_turf(src), buffer)
			user.put_in_hands(P)
			buffer = list()
	...
	if(href_list["create"])
		if(areactants.len)
			var/obj/item/nuclearfuel/rod/R = new(get_turf(src), areactants)
			user.put_in_hands(R)
			areactants = list()
			playsound(src.loc, 'sound/items/jaws_pry.ogg', 50, 1)
...
/obj/machinery/centrifuge/Process()
	...
	if(amount >= 20 && power(load))
		...
		spawn(50)
			var/obj/item/nuclearfuel/pellet/Pl = new(get_turf(src), list("U235" = 550, "U238" = 2250))
