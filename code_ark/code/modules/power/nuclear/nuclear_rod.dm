...
/obj/machinery/power/nuclear_rod
	...
	var/obj/item/nuclearfuel/rod/F = null
...
/obj/machinery/power/nuclear_rod/physical_attack_hand(mob/user)   //Removing the assembly.
	...
	if(reactants.len && do_after(user, 30,src) && rodtemp < 1000)
		...
		var/obj/item/nuclearfuel/rod/F = new(get_turf(src), reactants)
		user.put_in_hands(F)
		reactants = list()

/obj/machinery/power/nuclear_rod/attackby(obj/item/W, mob/user)  //Interaction with tools
	if(rodtemp < 2000)
		if(!broken)
			...
			else if(istype(W, /obj/item/nuclearfuel/rod))
